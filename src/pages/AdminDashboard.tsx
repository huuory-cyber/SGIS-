...
import { supabase, getRecords, getProviders, getStations, approveProvider, deactivateProvider } from '../lib/supabase';
...
import { SocialRecord, UserProfile, Station, Agent } from '../types';
import EnhancedForm from '../components/EnhancedForm';
import RecordsTable from '../components/RecordsTable';
import ProvidersManagement from '../components/ProvidersManagement';
import { rejectProvider } from '../lib/supabase';
...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'records' | 'providers' | 'stations' | 'form'>('overview');
  ...
  useEffect(() => {
    ...
    // Real-time subscription for social records
    const recordsChannel = supabase
      .channel('social_records_changes')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'social_records' }, (payload) => {
    ...
    // Real-time subscription for profiles (providers)
    const profilesChannel = supabase
      .channel('profiles_changes')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'profiles' }, (payload) => {
        if (payload.eventType === 'INSERT') {
          setProviders(prev => [payload.new as UserProfile, ...prev]);
        } else if (payload.eventType === 'UPDATE') {
          setProviders(prev => prev.map(p => p.id === payload.new.id ? payload.new as UserProfile : p));
        } else if (payload.eventType === 'DELETE') {
  ...
  const loadData = async () => {
    ...
    try {
      const [recordsData, providersData, stationsData] = await Promise.all([
        getRecords(),
        getProviders(),
        getStations(),
      ]);
      console.log('Dados carregados:', { records: recordsData?.length, providers: providersData?.length, stations: stationsData?.length });
      setRecords(recordsData as SocialRecord[]);
      setProviders(providersData as UserProfile[]);
  ...
  const handleExportAll = () => {
    exportRecordsToExcel(records, `sgis-registros-completos-${format(new Date(), 'yyyy-MM-dd')}.xlsx`);
  };
  ...
  const handleExportWeekly = () => {
    ...
    exportRecordsToExcel(weeklyRecords, `sgis-registros-semana-${format(new Date(), 'yyyy-MM-dd')}.xlsx`);
  };

  const handleExportMonthly = () => {
    const oneMonthAgo = new Date();
    oneMonthAgo.setMonth(oneMonthAgo.getMonth() - 1);
    const monthlyRecords = records.filter(r => new Date(r.created_at) >= oneMonthAgo);
    exportRecordsToExcel(monthlyRecords, `sgis-registros-mes-${format(new Date(), 'yyyy-MM')}.xlsx`);
  };

  const handleExportYearly = () => {
    const startOfYear = new Date(new Date().getFullYear(), 0, 1);
    const yearlyRecords = records.filter(r => new Date(r.created_at) >= startOfYear);
    exportRecordsToExcel(yearlyRecords, `sgis-registros-ano-${new Date().getFullYear()}.xlsx`);
  };
          ...
          <nav className="flex-1 space-y-2">
            <button 
              ...
              className={cn(
                ...
                activeTab === 'overview' ? "bg-blue-500 text-white shadow-lg shadow-blue-500/20" : "text-slate-400 hover:text-white hover:bg-slate-800"
              )}
            >
              <BarChart3 size={20} />
            ...
            <button 
              ...
              className={cn(
                "w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all",
                activeTab === 'providers' ? "bg-blue-500 text-white shadow-lg shadow-blue-500/20" : "text-slate-400 hover:text-white hover:bg-slate-800"
              )}
            >
              <Building size={20} />
              <span className="font-medium">Provedores</span>
            ...
            <button 
              ...
              className={cn(
                "w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all",
                activeTab === 'stations' ? "bg-blue-500 text-white shadow-lg shadow-blue-500/20" : "text-slate-400 hover:text-white hover:bg-slate-800"
              )}
            >
              <MapPin size={20} />
              <span className="font-medium">Postos</span>
            </button>
            <button 
          ...
          <div className="pt-6 border-t border-slate-800">
            <div className="flex items-center gap-3 px-4 py-3 text-slate-400">
              ...
              <div className="flex-1 overflow-hidden">
                ...
                <p className="text-xs truncate text-blue-400">Administrador</p>
              </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-lg font-semibold text-slate-800 capitalize">
              {activeTab === 'overview' ? 'Visão Geral' : 
               ...
               activeTab === 'providers' ? 'Gestão de Provedores' :
               activeTab === 'stations' ? 'Gestão de Postos' : 'Novo Registro'}
            </h2>
          </div>
          <div className="flex items-center gap-4">
          ...
          <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {[
                    ...
                    { label: 'Provedores Ativos', value: providers.filter(p => p.is_active).length, icon: Building, color: 'emerald' },
                  ].map((stat, i) => (
                ...
                <div className="mb-6 bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
                  <div className="flex items-center justify-between flex-wrap gap-4">
                    <div>
                      ...
                      <p className="text-sm text-slate-500">Baixe os registros em diferentes períodos</p>
                    </div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-600 rounded-xl hover:bg-emerald-100 transition-all text-sm font-medium"
                      >
                        <Download size={16} />
                        Mensal
                      </button>
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-purple-50 text-purple-600 rounded-xl hover:bg-purple-100 transition-all text-sm font-medium"
                      >
                        <Download size={16} />
                        Anual
                      </button>
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-slate-800 text-white rounded-xl hover:bg-slate-700 transition-all text-sm font-medium"
                      >
                        <Download size={16} />
                        Todos
                      </button>
            ...
            {activeTab === 'providers' && (
              <motion.div 
                ...
                exit={{ opacity: 0 }}
              >
                <ProvidersManagement providers={providers} onRefresh={loadData} adminId={user?.id || ''} />
              </motion.div>
            )}
            ...
            {activeTab === 'stations' && (
              <motion.div 
                ...
                exit={{ opacity: 0 }}
              >
                <StationsManagement stations={stations} providers={providers} onRefresh={loadData} />
              </motion.div>
            )}
